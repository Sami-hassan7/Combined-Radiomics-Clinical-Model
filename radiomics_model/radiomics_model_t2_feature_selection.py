...
def main():
  ...
  # Load data
  df = pd.read_csv('/home1/p303924/MRI_umcg_new/Data/OPCdigits_t1t2_split.csv')  
  ...
  ################### radiomics part
  radiodata = pd.read_csv('/home1/p303924/MRI_umcg_new/radiomics/umcg_radiomicsfeature_t2_GTVpt.csv', index_col = 'Unnamed: 0')   
  radiomics_feature_list = radiodata.columns.tolist()[1:]
  
  # Merge the two dataframes based on the patient ID column 
  merged_data = pd.merge(df,radiodata, on='PatientID') 
  merged_data.to_csv('/home1/p303924/MRI_umcg_new/radiomics/' + 'umcg_data_t2_GTVpt_' + event_vars[opt.outcome] + '.csv') 
  ...
  if 1 == 1:
      ...
      Feature_freq_sorted.to_csv('/home1/p303924/MRI_umcg_new/radiomics/radiomics_freq_t2_gtvpt_' + event_vars[opt.outcome] + '.csv', index = True, header=True) 
