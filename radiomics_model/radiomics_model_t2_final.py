...
def main():
  ...
  # load settings
  # use the desired outcome numbers
  opt = parse_opts()
  opt.outcome = 5
  ...
  ################### radiomics part
  radiodata = pd.read_csv('/Intern Programming/data/output/umcg_radiomicsfeature_t2_GTVPT.csv', index_col = 'Unnamed: 0') 
  radiomics_feature_list = radiodata.columns.tolist()[37:]

# Other erosion data 1mm, 3mm and 5mm 
  """
  radiodata = pd.read_csv('/Intern Programming/data/output/umcg_radiomicsfeature_t2_GTVPTtot_1mm.csv', index_col = 'Unnamed: 0') 
  radiomics_feature_list = radiodata.columns.tolist()[37:]

  radiodata = pd.read_csv('/Intern Programming/data/output/umcg_radiomicsfeature_t2_GTVPTtot_3mm.csv', index_col = 'Unnamed: 0') 
  radiomics_feature_list = radiodata.columns.tolist()[37:]

  radiodata = pd.read_csv('/Intern Programming/data/output/umcg_radiomicsfeature_t2_GTVPTtot_5mm.csv', index_col = 'Unnamed: 0') 
  radiomics_feature_list = radiodata.columns.tolist()[37:]
  ...
  if 1 == 1:    
      ...
      print ('clc test c-index ', cindex) 
  
  
          

  # radiomics model for oroginal gtvpt t2 Original
  if opt.outcome == 0:
      radio_predictor = [ 'original_glrlm_RunEntropy']  
  if opt.outcome == 1:
      radio_predictor = [ 'original_glszm_SizeZoneNonUniformityNormalized']    
  if opt.outcome == 2:
      radio_predictor = [ 'original_firstorder_Range' ]    
  if opt.outcome == 3:
      radio_predictor = ['original_glszm_ZoneEntropy' ]  
  if opt.outcome == 4:
      radio_predictor = ['original_glszm_SizeZoneNonUniformityNormalized', 'original_glrlm_RunEntropy' ] 
  if opt.outcome == 5:
      radio_predictor = ['original_glszm_ZoneEntropy']  
  if opt.outcome == 6:
      radio_predictor = ['original_glszm_SizeZoneNonUniformityNormalized', 'original_glrlm_RunEntropy' ]        
  ...
  """
      # radiomics model for oroginal gtvpt t2 1mm 
  if opt.outcome == 0:
      radio_predictor = [ 'original_glszm_ZoneEntropy']  
  if opt.outcome == 1:
      radio_predictor = [ 'original_gldm_SmallDependenceLowGrayLevelEmphasis']    
  if opt.outcome == 2:
      radio_predictor = [ 'original_gldm_DependenceVariance' ]    
  if opt.outcome == 3:
      radio_predictor = ['original_glszm_ZoneEntropy', 'original_gldm_SmallDependenceLowGrayLevelEmphasis' ]  
  if opt.outcome == 4:
      radio_predictor = ['original_gldm_SmallDependenceLowGrayLevelEmphasis', 'original_glszm_ZoneEntropy' ] 
  if opt.outcome == 5:
      radio_predictor = ['original_glszm_ZoneEntropy', 'original_gldm_DependenceVariance', 'original_gldm_SmallDependenceLowGrayLevelEmphasis']  
  if opt.outcome == 6:
      radio_predictor = ['original_gldm_SmallDependenceLowGrayLevelEmphasis', 'original_gldm_DependenceVariance' ]        
  if opt.outcome == 7:
      radio_predictor = ['original_gldm_LowGrayLevelEmphasis' ] 
  
      

       # radiomics model for oroginal gtvpt t2 3mm
  if opt.outcome == 0:
      radio_predictor = [ 'original_ngtdm_Coarseness']  
  if opt.outcome == 1:
      radio_predictor = [ 'original_glrlm_RunEntropy']    
  if opt.outcome == 2:
      radio_predictor = [ 'original_glcm_SumAverage' ]    
  if opt.outcome == 3:
      radio_predictor = ['original_ngtdm_Coarseness', 'original_glrlm_RunEntropy' ]  
  if opt.outcome == 4:
      radio_predictor = ['original_glrlm_RunEntropy', 'original_glcm_SumAverage' ] 
  if opt.outcome == 5:
      radio_predictor = ['original_ngtdm_Coarseness', 'original_glrlm_RunEntropy', 'original_glcm_SumAverage']  
  if opt.outcome == 6:
      radio_predictor = ['original_glrlm_RunEntropy', 'original_glcm_SumAverage' ]        
  if opt.outcome == 7:
      radio_predictor = ['original_glcm_Idn' ] 


       # radiomics model for oroginal gtvpt t2 5mm
  if opt.outcome == 0:
      radio_predictor = [ 'original_glcm_Idmn']  
  if opt.outcome == 1:
      radio_predictor = [ 'original_glszm_SizeZoneNonUniformityNormalized']    
  if opt.outcome == 2:
      radio_predictor = [ 'original_glszm_GrayLevelNonUniformityNormalized' ]    
  if opt.outcome == 3:
      radio_predictor = ['original_glcm_Idmn', 'original_glszm_SizeZoneNonUniformityNormalized' ]  
  if opt.outcome == 4:
      radio_predictor = ['original_glcm_Idmn', 'original_glszm_GrayLevelNonUniformityNormalized' ] 
  if opt.outcome == 5:
      radio_predictor = ['original_glcm_Idmn', 'original_glszm_SizeZoneNonUniformityNormalized', 'original_glszm_GrayLevelNonUniformityNormalized']  
  if opt.outcome == 6:
      radio_predictor = ['original_glszm_SizeZoneNonUniformityNormalized', 'original_glszm_GrayLevelNonUniformityNormalized' ]        
  if opt.outcome == 7:
      radio_predictor = ['original_glcm_SumAverage' ] 
